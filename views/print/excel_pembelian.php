...
$objPHPExcel->setActiveSheetIndex(0)
			...
			->setCellValue('H1', 'Nama item')
			->setCellValue('I1', 'Jumlah')
			->setCellValue('J1', 'Harga Pembelian/qty');
...
while ($row = mysql_fetch_array($query)) {
            ...
            ->setCellValue('E'.$i, 'Rp. '.number_format($row['purchase_total']))
            ->setCellValue('F'.$i, $row['supplier_name'])
			->setCellValue('G'.$i, $row['branch_name'])
			// ->setCellValue('H'.$i, 'Rp. '.number_format($svc))
			->setCellValue('H'.$i, $row['item_name'])
			->setCellValue('I'.$i, $row['purchase_qty'])
			->setCellValue('J'.$i, 'Rp. '.number_format($row['purchase_price']));
			$i++;
			$j++;
}
...
header('Content-Disposition: attachment;filename="Bengkel.xlsx"');
